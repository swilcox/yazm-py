from enums import OperandCount, OpForm, OpSize
...
def get_operand_count(opcode, form):
    ...
    if form == OpForm.EXT:
        return OperandCount.VAR
    return OperandCount.TWO # LongForm
...
def decode(zm):
    ...
    if form == OpForm.SHORT:
        ...
        szbyte = zm.memory[pc + 1]
        operand_ptr = pc + 2
        sizes = get_operand_sizes(szbyte)
        # handle call_vn2/vs2's extra szbyte
        if opcode in (236, 250):
            szbyte2 = zm.memory[pc + 2]
            sizes += get_operand_sizes(szbyte2)
            operand_ptr = pc + 3
    elif form == OpForm.EXT:
        szbyte = zm.memory[pc + 2]
        operand_ptr = pc + 3
        sizes = get_operand_sizes(szbyte)
    elif form == OpForm.LONG:
        operand_ptr = pc + 1
        sizes = []
        for offset in (6,5):
            if (opcode >> offset) & 1:
                sizes.append(OpSize.VAR)
            else:
                sizes.append(OpSize.BYTE)
    else:
        print('unknown opform specified: ' + str(form))
    ...
    for i in range(len(sizes)):
        size = sizes[i]
        if size == OpSize.WORD:
            operands.append(zm.memory.u16(operand_ptr))
            operand_ptr += 2
        elif size == OpSize.BYTE:
            operands.append(zm.memory[operand_ptr])
            operand_ptr += 1
        elif size == OpSize.VAR:
            operands.append(None) #this is fixedup after every load from icache
            var_num = zm.memory[operand_ptr]
            var_op_info.append((i, var_num))
            operand_ptr += 1
        else:
            print('unknown operand size specified: ' + str(size))
    ...
    if has_branch_var[opcode]: # std:4.7
        ...
        if branch_info & 64:
            ...
            branch_offset <<= 8
            branch_offset |= zm.memory[operand_ptr]
            operand_ptr += 1
    ...
    if form != OpForm.EXT and opcode in (178, 179):
        while True:
            word = zm.memory.u16(operand_ptr)
            operand_ptr += 2
